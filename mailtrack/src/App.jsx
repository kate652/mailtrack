...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Courier New', monospace" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Share+Tech+Mono&family=Barlow+Condensed:wght@300;400;600;700&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; }`}</style>
      <div style={{ width: "100%", maxWidth: 420, padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <div style={{ width: 48, height: 48, border: "2px solid #e8e4dc", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 22, marginBottom: 20 }}>✉</div>
          <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 32, fontWeight: 700, letterSpacing: 6, textTransform: "uppercase", color: "#e8e4dc" }}>MailTrack</div>
          <div style={{ fontSize: 10, color: "#444", letterSpacing: 3, marginTop: 6 }}>AI-POWERED POSTAL SYSTEM</div>
        </div>

        <div style={{ background: "#0d0d0d", border: "1px solid #1a1a1a", padding: 32 }}>
          <div style={{ fontSize: 11, color: "#555", letterSpacing: 3, textTransform: "uppercase", marginBottom: 28 }}>Sign In</div>

          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 10, color: "#555", letterSpacing: 2, marginBottom: 6, textTransform: "uppercase" }}>Email</div>
            <input
              type="email" value={email} onChange={e => setEmail(e.target.value)}
              onKeyDown={e => e.key === "Enter" && handleLogin()}
              placeholder="you@company.com"
              style={{ width: "100%", background: "#111", border: "1px solid #2a2a2a", color: "#e8e4dc", padding: "10px 12px", fontFamily: "'Share Tech Mono', monospace", fontSize: 12, outline: "none" }}
            />
          </div>

          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 10, color: "#555", letterSpacing: 2, marginBottom: 6, textTransform: "uppercase" }}>Password</div>
            <input
          ...
          {error && (
            <div style={{ background: "#2a0a0a", border: "1px solid #7f1d1d", color: "#fca5a5", padding: "10px 14px", fontSize: 11, letterSpacing: 1, marginBottom: 16, fontFamily: "'Share Tech Mono', monospace" }}>
              ✗ {error}
            </div>
          )}

          <button onClick={handleLogin} disabled={loading} style={{
            width: "100%", padding: "13px", background: loading ? "#1a1a1a" : "#e8e4dc",
            color: loading ? "#444" : "#0a0a0a", border: "none", cursor: loading ? "not-allowed" : "pointer",
            fontFamily: "'Barlow Condensed', sans-serif", fontSize: 14, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase",
          }}>
            {loading ? "◈ SIGNING IN..." : "⬛ SIGN IN"}
          </button>
        </div>

        <div style={{ textAlign: "center", marginTop: 20, fontSize: 10, color: "#333", letterSpacing: 1 }}>
...
export default function MailTracker() {
  ...
  if (!authChecked) return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center", color: "#555", fontFamily: "'Share Tech Mono', monospace", fontSize: 12, letterSpacing: 3 }}>
      <style>{`@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
      <span style={{ display: "inline-block", animation: "spin 1.5s linear infinite", marginRight: 10 }}>◈</span> LOADING...
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#e8e4dc", fontFamily: "'Courier New', monospace" }}>
      ...
      <header style={{ background: "#0d0d0d", borderBottom: "1px solid #1a1a1a", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ borderLeft: "1px solid #2a2a2a", paddingLeft: 12, display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 11, color: "#aaa", fontFamily: "'Share Tech Mono', monospace" }}>{userName}</div>
              <div style={{ fontSize: 9, color: "#444", letterSpacing: 1 }}>{userEmail}</div>
            </div>
            <button className="btn" onClick={signOut} style={{
              padding: "5px 12px", background: "transparent", color: "#555",
              border: "1px solid #2a2a2a", fontSize: 9, letterSpacing: 2, textTransform: "uppercase",
          ...
          <div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 8, alignItems: "center" }}>
                <input className="inp" placeholder="SEARCH SENDER, RECIPIENT, SUBJECT..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1 }} />
...
Sent from MailTrack`
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                {/* Status filter */}
                <div style={{ display: "flex", gap: 1, background: "#1a1a1a", flex: 1 }}>
                  {["All", ...STATUS_FLOW].map(f => (
                    <button key={f} onClick={() => setFilter(f)} className="tab btn" style={{
                      flex: 1, padding: "7px 10px", fontSize: 10, letterSpacing: 1, textTransform: "uppercase",
                      background: filter === f ? "#1e1e1e" : "transparent",
                      color: filter === f ? (STATUS_COLORS[f] || "#e8e4dc") : "#444",
                      borderBottom: filter === f ? `2px solid ${STATUS_COLORS[f] || "#e8e4dc"}` : "2px solid transparent",
                      border: "none",
                    }}>{f}</button>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", gap: 1, background: "#1a1a1a" }}>
                  {["All", "Low", "Medium", "High"].map(p => (
                    <button key={p} onClick={() => setPriorityFilter(p)} className="tab btn" style={{
                      padding: "7px 12px", fontSize: 10, letterSpacing: 1, textTransform: "uppercase",
                      background: priorityFilter === p ? "#1e1e1e" : "transparent",
                      color: priorityFilter === p ? (PRIORITY_COLORS[p] || "#e8e4dc") : "#444",
                      borderBottom: priorityFilter === p ? `2px solid ${PRIORITY_COLORS[p] || "#e8e4dc"}` : "2px solid transparent",
                      border: "none",
                    }}>{p === "All" ? "All Priority" : `${PRIORITY_ICONS[p]} ${p}`}</button>
                  ))}
                </div>

                {/* Clear filters — show only when active */}
                {(filter !== "All" || priorityFilter !== "All" || search) && (
                  <button className="btn" onClick={() => { setFilter("All"); setPriorityFilter("All"); setSearch(""); }} style={{
                    padding: "7px 12px", background: "transparent", color: "#555",
                    border: "1px solid #2a2a2a", fontSize: 10, letterSpacing: 2,
                    textTransform: "uppercase", fontFamily: "'Share Tech Mono', monospace", whiteSpace: "nowrap"
                  }}>✕ CLEAR</button>
                )}
              </div>
            </div>
                ...
                <div style={{ border: "1px solid #1a1a1a", overflowX: "auto" }}>
                  ...
                  {filtered.map(mail => (
                    <div key={mail.id} className="mail-row"
                      ...
                      <span style={{ fontSize: 12, color: "#888", paddingLeft: 16, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{mail.sender}</span>
                      ...
                      <span style={{ fontSize: 10, color: PRIORITY_COLORS[mail.priority || "Medium"], background: `${PRIORITY_COLORS[mail.priority || "Medium"]}15`, padding: "3px 8px", letterSpacing: 1, whiteSpace: "nowrap", display: "inline-block" }}>{PRIORITY_ICONS[mail.priority || "Medium"]} {(mail.priority || "Medium").toUpperCase()}</span>
        ...
        {view === "upload" && (
          <div style={{ maxWidth: 680, margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <div>
                <div style={{ fontSize: 10, color: "#555", letterSpacing: 2, marginBottom: 8, textTransform: "uppercase" }}>Priority</div>
                <div style={{ display: "flex", gap: 1, background: "#1a1a1a" }}>
                  {["Low", "Medium", "High"].map(p => (
                    <button key={p} onClick={() => setForm(f => ({ ...f, priority: p }))} className="btn" style={{
                      flex: 1, padding: "10px", fontSize: 11, letterSpacing: 2, textTransform: "uppercase",
                      fontFamily: "'Share Tech Mono', monospace",
                      background: form.priority === p ? `${PRIORITY_COLORS[p]}18` : "transparent",
                      color: form.priority === p ? PRIORITY_COLORS[p] : "#444",
                      borderBottom: form.priority === p ? `2px solid ${PRIORITY_COLORS[p]}` : "2px solid transparent",
                      border: "none",
                    }}>
                      {PRIORITY_ICONS[p]} {p}
                    </button>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: "#1a1a1a", marginBottom: 20 }}>
                    ...
                    <div style={{ background: "#0d0d0d", padding: "14px 16px" }}>
                      <div style={{ fontSize: 9, color: "#444", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>Priority</div>
                      <div style={{ display: "flex", gap: 6 }}>
                        {["Low", "Medium", "High"].map(p => (
                          <button key={p} className="btn" onClick={async () => {
                            try {
                              await sbFetch(`/mails?id=eq.${encodeURIComponent(mail.id)}`, { method: "PATCH", body: JSON.stringify({ priority: p }) });
                            ...
                            fontFamily: "'Share Tech Mono', monospace",
                            background: (mail.priority || "Medium") === p ? `${PRIORITY_COLORS[p]}22` : "transparent",
                            color: (mail.priority || "Medium") === p ? PRIORITY_COLORS[p] : "#444",
                            border: `1px solid ${(mail.priority || "Medium") === p ? PRIORITY_COLORS[p] + "66" : "#2a2a2a"}`,
                            cursor: "pointer",
                    ...
                    {[...mail.history].reverse().map((h, i) => (
                      <div key={i} style={{ display: "flex", gap: 16, marginBottom: 20 }}>
                        ...
                        <div style={{ paddingBottom: 16 }}>
                          ...
                          <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>{formatDate(h.time)}{h.by ? ` · ${h.by}` : ""}</div>
                  ...
                  <div style={{ borderTop: "1px solid #1a1a1a", paddingTop: 24 }}>
                    ...
                    <div style={{ background: "#0a0a0a", border: "1px solid #1e1e1e", padding: 14 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                        <div style={{ width: 24, height: 24, background: "#1a1a1a", border: "1px solid #2a2a2a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#60a5fa", flexShrink: 0, fontFamily: "'Share Tech Mono', monospace" }}>
                          {userName.charAt(0).toUpperCase()}
                        </div>
                        <span style={{ fontSize: 11, color: "#aaa", letterSpacing: 1 }}>{userName}</span>
                        <span style={{ fontSize: 9, color: "#333", letterSpacing: 1, marginLeft: "auto" }}>POSTING AS SIGNED-IN USER</span>
                      </div>
                      <textarea
