...
async function generateTrackingNumber() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(rows) && rows.length > 0 && typeof rows[0].value === "number") {
  ...
  // Fallback: derive from highest existing mail ID
  try {
    const res = await fetch(`${SB_URL}/rest/v1/mails?select=id&order=created_at.desc&limit=200`, {
      headers: { "apikey": SB_KEY, "Authorization": `Bearer ${SB_KEY}` }
    });
    if (res.ok) {
      const rows = await res.json();
...
export default function MailTracker() {
  ...
  async function handleSubmit() {
    ...
    if (!pendingId) {
      showToast("Tracking ID not ready yet — please wait a moment.", "warn");
      return;
    }
    const id = pendingId;
          ...
          <div style={{ maxWidth: 680, margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
                <button className="btn" onClick={handleSubmit} disabled={scanning || !pendingId} style={{
                  flex: 1, padding: "14px", background: scanning || !pendingId ? "#1a1a1a" : "#e8e4dc", color: scanning || !pendingId ? "#444" : "#0a0a0a",
                  fontSize: 13, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase",
                  fontFamily: "'Barlow Condensed', sans-serif", cursor: scanning || !pendingId ? "not-allowed" : "pointer"
                }}>{scanning ? "◈ SCANNING..." : !pendingId ? "◈ GENERATING ID..." : "⬛ GENERATE TRACKING & LOG MAIL"}</button>
